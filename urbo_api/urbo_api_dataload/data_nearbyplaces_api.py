...
router = APIRouter(
    tags=["nearby-places"], responses={404: {"description": "Not Found"}}
)


@router.post("/fetch-nearby-places/", response_model=schema.NearbyPlaceResponse)
def fetch_nearby_places(
    place_data: schema.NearbyPlacesCreate, db_session: Session = Depends(get_db)
):
    """
    ...
    if response.status_code == 200:
        ...
        ref_location_point = WKTElement(
            f"POINT({place_data.ref_location[1]} {place_data.ref_location[0]})",
            srid=4326,
        )
        ...
        raise HTTPException(
            status_code=400, detail="Failed to fetch places from Mapple API"
        )
